...
const ProjectList = () => {
  ...
  const fetchProjects = async () => {
    try {
      const response = await fetch(`http://ec2-13-234-31-37.ap-south-1.compute.amazonaws.com:8181/projects?page=${page}&limit=${limit}&search=${encodeURIComponent(search)}`);
      if (!response.ok) {
  ...
  const handleDelete = async () => {
    try {
      const response = await fetch(`http://ec2-13-234-31-37.ap-south-1.compute.amazonaws.com:8181/projects/${projectToDelete._id}`, {
        method: 'DELETE',
      });
  ...
  const handleUpdate = async () => {
    try {
      const response = await fetch(`http://ec2-13-234-31-37.ap-south-1.compute.amazonaws.com:8181/projects/${projectToUpdate._id}`, {
        method: 'PUT',
        headers: {
