...
const EditInvoice = () => {
  ...
  const fetchInvoices = async (pageNumber) => {
    try {
      const response = await fetch(`http://ec2-52-66-244-210.ap-south-1.compute.amazonaws.com:8181/invoices?page=${pageNumber}&limit=${limit}&search=${encodeURIComponent(search)}`);
      if (!response.ok) {
  ...
  const fetchInvoiceItemsByInvID = async (invID) => {
    try {
      const response = await fetch(`http://ec2-52-66-244-210.ap-south-1.compute.amazonaws.com:8181/getInvoiceItemByInvID/${invID}`);
      if (!response.ok) {
  ...
  const handleDelete = async () => {
    try {
      const response = await fetch(`http://ec2-52-66-244-210.ap-south-1.compute.amazonaws.com:8181/invoices/${invoiceToDelete._id}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
    ...
    try {
      const promises = updateItems.map(async (item) => {
        if (item.invtID) {
          ...
          const response = await fetch(`http://ec2-52-66-244-210.ap-south-1.compute.amazonaws.com:8181/invoiceItem/${item.invtID}`, {
            method: 'PUT',
          ...
          const response = await fetch(`http://ec2-52-66-244-210.ap-south-1.compute.amazonaws.com:8181/invoiceItem`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              itemDesc: item.itemDesc,
  ...
  const handleDeleteItem = async (invtID, index) => {
    ...
    const apiUrl = `http://ec2-52-66-244-210.ap-south-1.compute.amazonaws.com:8181/invoiceItem/${numericInvtID}`;
