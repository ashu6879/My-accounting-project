...
const EditInvoice = () => {
  ...
  const fetchInvoices = async (pageNumber) => {
    try {
      const response = await fetch(`https://my-accounting-u7vs.onrender.com/invoices?page=${pageNumber}&limit=${limit}&search=${encodeURIComponent(search)}`);
      if (!response.ok) {
  ...
  const fetchInvoiceItemsByInvID = async (invID) => {
    try {
      const response = await fetch(`https://my-accounting-u7vs.onrender.com/getInvoiceItemByInvID/${invID}`);
      if (!response.ok) {
  ...
  const handleDelete = async () => {
    try {
      const response = await fetch(`https://my-accounting-u7vs.onrender.com/invoices/${invoiceToDelete._id}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
    ...
    try {
      const promises = updateItems.map(async (item) => {
        if (item.invtID) {
          ...
          const response = await fetch(`https://my-accounting-u7vs.onrender.com/invoiceItem/${item.invtID}`, {
            method: 'PUT',
          ...
          const response = await fetch(`https://my-accounting-u7vs.onrender.com/invoiceItem`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              itemDesc: item.itemDesc,
  ...
  const handleDeleteItem = async (invtID, index) => {
    ...
    const apiUrl = `https://my-accounting-u7vs.onrender.com/invoiceItem/${numericInvtID}`;
