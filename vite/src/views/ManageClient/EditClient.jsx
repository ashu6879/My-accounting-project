...
const CategoryList = () => {
  ...
  const fetchClients = async () => {
    try {
      const response = await fetch(`http://localhost:81/clients?page=${page}&limit=${limit}&search=${encodeURIComponent(search)}`);
      if (!response.ok) {
  ...
  const handleDelete = async () => {
    try {
      const response = await fetch(`http://localhost:81/clients/${clientToDelete._id}`, {
        method: 'DELETE',
      });
  ...
  const handleUpdate = async () => {
    try {
      const response = await fetch(`http://localhost:81/clients/${clientToUpdate._id}`, {
        method: 'PUT',
        headers: {
