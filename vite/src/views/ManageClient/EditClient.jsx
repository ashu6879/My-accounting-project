...
const CategoryList = () => {
  ...
  const fetchClients = async () => {
    try {
      const response = await fetch(`https://my-accounting-u7vs.onrender.com/clients?page=${page}&limit=${limit}&search=${encodeURIComponent(search)}`);
      if (!response.ok) {
  ...
  const handleDelete = async () => {
    try {
      const response = await fetch(`https://my-accounting-u7vs.onrender.com/clients/${clientToDelete._id}`, {
        method: 'DELETE',
      });
  ...
  const handleUpdate = async () => {
    try {
      const response = await fetch(`https://my-accounting-u7vs.onrender.com/clients/${clientToUpdate._id}`, {
        method: 'PUT',
        headers: {
